...
**en tu navegador** usando WebGPU con el modelo de visión **Qwen2-VL-2B** (Transformers.js).
...
1. Pulsa **Cargar modelo** (primera vez descarga ~2.5 GB, luego queda cacheado en el navegador).
2. Sube una imagen, escribe el contexto y pulsa **Describir imagen**.

> El modelo Qwen2-VL-2B entiende mejor el inglés; escribe el contexto en inglés para
> mejores resultados.
...
├── templates/
│   └── index.html         # UI + lógica WebGPU (Qwen2-VL-2B)
└── requirements.txt
```